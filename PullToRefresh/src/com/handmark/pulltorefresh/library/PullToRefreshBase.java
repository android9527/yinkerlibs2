...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public final void onRefreshSuccess() {
        if (isRefreshing()) {
            onHeadRefreshSuccess();
            if (refreshHandler == null) {
                refreshHandler = new RefreshHandler(PullToRefreshBase.this);
            }
            refreshHandler.sendEmptyMessageDelayed(RefreshHandler.MESSAGE_ON_REFRESH_COMPLETE, 500);
        }
    }

    private void onHeadRefreshSuccess() {
        switch (mCurrentMode) {
            case PULL_FROM_END:
                mFooterLayout.onHeadRefreshSuccess();
                break;
            case PULL_FROM_START:
                mHeaderLayout.onHeadRefreshSuccess();
                break;
            default:
                // NO-OP
                break;
...
//            e.printStackTrace();
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrHeaderBackground)) {
            int background = a.getColor(R.styleable.PullToRefresh_ptrHeaderBackground, 0xFFFFFF);
            if (0 != background) {
                mHeaderLayout.setBackgroundColor(background);
            }
        }
    ...
    /**
     */
    private static final class RefreshHandler extends BaseHandler<PullToRefreshBase> {

        public static final int MESSAGE_ON_REFRESH_COMPLETE = 1;

        RefreshHandler(PullToRefreshBase pullToRefreshBase) {
            super(pullToRefreshBase);
        }

        @Override
        public void onHandleMessage(Message msg, PullToRefreshBase pullToRefreshBase) {
            switch (msg.what) {
                case MESSAGE_ON_REFRESH_COMPLETE:
